...
for val in range (len(human_files_short)):
    ...
    if(is_face_detected):
        dog_inHumans += 1
print("%d in human_files_short" % dog_inHumans)

for val in range (len(dog_files_short)):
    is_face_detected = dog_detector(dog_files_short[val])
    if(is_face_detected):
        dog_inDogs += 1
print("%d in human_files_short" % dog_inDogs)
...
def predictor(path):
    ...
    print('Dog bread is {}'.format(breeds))
    plt.imshow(image)
...
# 3. 增强训练图像的质量
# 
# BTW: 直接用的Udacity提供的继承jupyter,无法上传自己的照片，所以只提供了一个人类照片，在本地已经测试了几张人类照片。

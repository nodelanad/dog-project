...
human_inHumans = 0
human_inDogs = 0
for val in range (len(human_files_short)):
    is_face_detected = face_detector(human_files_short[val])
    if(is_face_detected):
        human_inHumans += 1
print("%s %% in human_files " % human_inHumans)

for val in range (len(dog_files_short)):
    is_face_detected = face_detector(dog_files_short[val])
    if(is_face_detected):
        human_inDogs += 1
print("%s %% in dog_files " % human_inDogs)
...
# 这是合理的。因为基于haarcascades模型是使用含有清晰面部特征的人脸进行训练的，如果没有清晰的面部特征那么检测效果不佳。如需改进，我们可以让模型同时训练大量不清晰的面部特征的图像，提高检测水平。或者基于如openface等的已有数据集去进行检测。
...
dog_inDogs = 0
for val in range (len(human_files_short)):
    is_face_detected = dog_detector(human_files_short[val])
    if(is_face_detected):
        dog_inHumans += 1
print("%s %% in human_files_short" % dog_inHumans)

for val in range (len(dog_files_short)):
    is_face_detected = dog_detector(dog_files_short[val])
    if(is_face_detected):
        dog_inDogs += 1
print("%s %% in human_files_short" % dog_inDogs)
...
# 
# __回答:__ 
# 1. 我们使用三层卷积层用于特征提取，由于“relu”函数具有梯度下降快且降低梯度小时，计算相对“sigmoid”函数更加简单等特点，本次使用他作为激活函数
# 2. 由于卷积网络会产生大量参数，为了减少训练参数数量，增加一层池化层用于降低卷积层特征维度，因最大池变现更好一点，本次采用了最大池化
# 3. 为了降低过拟合的状况，在池化层之后增加了Dropout()层
# 4. 用Flatten层将三维数据压缩成一维数据，用于卷积层和全连接层之间的连接
# 5. 因为是多分类问题，所以最后一层基于"softmax"建立133种狗类的输出
...
### TODO: 定义你的网络架构
model.add(Conv2D(filters=16, input_shape=(224, 224, 3), kernel_size=2, padding='same', activation='relu'))
...
# 
# __回答:__ 
# 1. 选择Inception V3的原因是因为权重小于VGG和ResNet，这意味着它占用较低内存。
# 
# 
...
InceptionV3_model.add(Dense(150, activation='relu', kernel_regularizer=regularizers.l2(0.0047)))
...
def predictor(path):
    breeds = InceptionV3_predict_breed(path)
    image  = mpimg.imread(path)
    
    if(dog_detector(path)):
        print('Dog')
        print('The bread of dog is {}'.format(breeds))
    elif(face_detector(path)):
        print('Human')
        print('human look like the bread of dog is {}'.format(breeds))
    else:
        raise ValueError('Not Human or Dog')
    plt.imshow(image)
...
# 回答：测试比我预期的好，测试的狗都猜对了，特别是美国水猎犬，我自己都猜不出来。
# 
# 改进：
# 1. 更多的数据
# 2. 调整参数和改变优化器
# 3. 增强训练图像的质量
# 
# ## BTW: 直接用的Udacity Workspace提供的jupyter,无法上传自己的照片，所以只提供了一个人类照片，在本地已经测试了几张人类照片。
# ## Review给出的上传upload没有找到 在jupyter里
...
## 自由地使用所需的代码单元数吧
predictor('images/sample_human_output.png')


# In[49]:


predictor('images/Labrador_retriever_06457.jpg')


# In[74]:


predictor('images/American_water_spaniel_00648.jpg')


# In[75]:


predictor('images/Brittany_02625.jpg')


# In[77]:


predictor('images/Curly-coated_retriever_03896.jpg')


# In[79]:


predictor('images/Labrador_retriever_06455.jpg')
